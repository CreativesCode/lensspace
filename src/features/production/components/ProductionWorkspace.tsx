...
export function ProductionWorkspace({ initialJobs, orders, providers }: { initialJobs: ProductionJob[]; orders: AssignmentOrder[]; providers: Provider[] }) {
  ...
  return <div className="space-y-6">
    {orders.length && providers.length ? <form onSubmit={assign} className="grid gap-3 rounded-[10px] border border-[#E3EFED] bg-white p-5 md:grid-cols-4"><h2 className="font-display text-lg font-semibold text-[#07322F] md:col-span-4">Asignar producción</h2><FormSelect name="orderId" ariaLabel="Pedido" value={String(selectedOrderId)} onValueChange={(value) => setSelectedOrderId(Number(value))} options={orders.map((order) => ({ value: String(order.id), label: order.orderNumber }))} /><FormSelect ariaLabel="Tipo de trabajo" value={jobType} onValueChange={(value) => setJobType(value as 'lens' | 'mounting')} options={[{ value: 'lens', label: 'Cristales' }, { value: 'mounting', label: 'Montaje' }]} /><FormSelect name="providerId" ariaLabel="Proveedor" required options={[{ value: '', label: 'Selecciona proveedor' }, ...matchingProviders.map((provider) => ({ value: provider.id, label: provider.name }))]} /><button disabled={pending || !matchingProviders.length} className="rounded-[7px] bg-[#0D7A72] px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40">Asignar trabajo</button></form> : null}
