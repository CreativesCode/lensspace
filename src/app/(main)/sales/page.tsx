...
export default async function SalesPage() {
  ...
  const { data: rawMemberships } = await supabase.from('organization_memberships').select('organization_id, branch_id').eq('user_id', user.id).eq('status', 'active').in('role', ['owner', 'seller']).not('branch_id', 'is', null)
  const memberships = (rawMemberships ?? []) as Pick<Tables<'organization_memberships'>, 'organization_id' | 'branch_id'>[]
  const organizationIds = [...new Set(memberships.map((entry) => entry.organization_id))]
  const branchIds = [...new Set(memberships.map((entry) => entry.branch_id).filter((id): id is number => id !== null))]
  const empty = Promise.resolve({ data: [] })
  const [{ data: organizationData }, { data: branchData }, { data: customerData }, { data: revisionData }, { data: itemData }] = await Promise.all([
    organizationIds.length ? supabase.from('organizations').select('id, name').in('id', organizationIds) : empty,
    branchIds.length ? supabase.from('branches').select('id, organization_id, name').in('id', branchIds) : empty,
    branchIds.length ? supabase.from('customers').select('id, organization_id, branch_id, full_name').in('branch_id', branchIds).is('archived_at', null).order('full_name').limit(200) : empty,
    branchIds.length ? supabase.from('prescription_revisions').select('id, organization_id, branch_id, prescription_id, prescription_date, prescriptions(customer_id)').in('branch_id', branchIds).order('created_at', { ascending: false }).limit(200) : empty,
  ...
  const organizations = await Promise.all(memberships.map(async (membership) => {
    const org = organizationRows.find((entry) => entry.id === membership.organization_id)
    const branch = branchRows.find((entry) => entry.id === membership.branch_id)
    const { data: canOperate } = await supabase.rpc('current_user_can_operate_organization', { target_organization_id: membership.organization_id, required_module_key: 'optical_sales' } as never)
    return { id: membership.organization_id, name: org?.name ?? 'Organización', branchId: membership.branch_id as number, branchName: branch?.name ?? 'Sucursal', canOperate: Boolean(canOperate) }
  }))
