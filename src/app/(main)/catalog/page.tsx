...
export default async function CatalogPage() {
  ...
  const [{ data: items }, { data: overrides }, { data: rules }, { data: revisions }] = await Promise.all([
    ...
    supabase.from('catalog_item_overrides').select('*'),
    supabase.from('graduation_rules').select('*').eq('is_active', true).order('name'),
    supabase.from('prescription_revisions').select('id, organization_id, prescription_id, revision_number, prescription_date').order('created_at', { ascending: false }).limit(100),
  ])
  ...
  return (
    <section className="mx-auto max-w-[1440px] px-5 py-8 lg:px-8">
      <div className="mb-6"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#0D7A72]">Ventas ópticas</p><h1 className="mt-2 font-display text-3xl font-bold tracking-tight text-[#07322F]">Catálogo y precios</h1><p className="mt-2 text-sm text-[#74857F]">Configura precios por organización y simula combinaciones sin perder la moneda de origen.</p></div>
      <CatalogWorkspace
